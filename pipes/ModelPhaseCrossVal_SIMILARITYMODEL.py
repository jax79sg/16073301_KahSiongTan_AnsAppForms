...
if __name__ == "__main__":
    if(len(sys.argv)==6 or len(sys.argv)==9):
        ...
        counter=0
        for splittedFeature in (glob.iglob(testSampleFilename + '.0*')):
            counter=counter+1

        for i in range(1, counter):
            newCVFilename = testSampleFilename.split('.')[0] + '.features.' + str(i).zfill(3)
            ...
            simModel.flushTestData()
            simModel.loadXYtest(newCVFilename)
            eval = Evaluator.Evaluator(utilObj=util)
            resultsAcc, resultsPrec, resultsRecall, resultsF1, resultsPrecperclass, resultsRecallperclass, resultsF1perclass = simModel.evaluate(
